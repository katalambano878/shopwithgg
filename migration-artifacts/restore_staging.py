#!/usr/bin/env python3
"""Restore Jane's Luxe dump into shopwithgg_staging (or janesluxe) on fleet-postgres."""
import json
import os
import subprocess
import sys
...
DUMP = Path(os.environ.get("SWGG_DUMP_DIR", "/data/coolify/shopwithgg-staging/dumps"))
DB = os.environ.get("SWGG_DB_NAME", "shopwithgg_staging")
ROLE = os.environ.get("SWGG_DB_ROLE", DB)
    ...
    for table in ("site_settings", "store_settings"):
        try:
            psql(
                f"""
                UPDATE public.{table}
                SET value = replace(value::text, 'https://iwgrqyemptmegyutnkfi.supabase.co', '')::jsonb
